...
void * watek_klient (void * arg_wsk){
  ...
  int i, i_wypite, kran = -1, kufel = -1, result;
  ...
  for(i_wypite=0; i_wypite<ile_musze_wypic; i_wypite++){
    ...
    do {
      for (int k=0; k<pub_wsk.l_wkf; k++) {
        if (pthread_mutex_trylock(&pub_wsk.tab_kuf[k]) == 0) {
          kufel = k;
          break;
        }
      }
    ...
    do {
      for (int k = 0; k < pub_wsk.l_kr; k++) {
        if (pthread_mutex_trylock(&pub_wsk.tab_kran[k]) == 0) {
          kran = k;
          break;
        }
      }
      if (kran == -1) {
        printf("\nKlient %d, brak wolnych kranow, czekam\n", moj_id);
        usleep(50);
      }
    } while (kran == -1);
