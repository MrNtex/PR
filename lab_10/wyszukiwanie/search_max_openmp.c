...
#pragma omp task default(none) firstprivate(A,p_task,k_task) shared(a_max)
    ...
    for(int i = p_task + 1; i <= k_task; i++) {
      if(A[i] > a_max_local) a_max_local = A[i];
    }
...
#pragma omp task default(none) firstprivate(A, p, mid, level) shared(max_left)
  {
    max_left = bin_search_max_task(A, p, mid, level + 1);
  }

#pragma omp task default(none) firstprivate(A, r, mid, level) shared(max_right)
  {
